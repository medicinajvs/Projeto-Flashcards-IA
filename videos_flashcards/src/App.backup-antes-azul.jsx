...
function normalizeFlashcards(rawFlashcards) {
  ...
  return rawFlashcards.map((card, index) => {
    ...
    return {
      ...
      sourceOrigin: card.sourceOrigin || card.source_origin || card.source_type || '',
      source_origin: card.source_origin || card.sourceOrigin || card.source_type || '',
      preceptorNote:
        card.preceptorNote ??
        card.nota_preceptor ??
        card.preceptor_note ??
...
function addFlashcardSource(cards = [], sourceOrigin = 'original') {
  return normalizeFlashcards(cards).map((card, index) => ({
    ...card,
    sourceOrigin: card.sourceOrigin || card.source_origin || sourceOrigin,
    source_origin: card.source_origin || card.sourceOrigin || sourceOrigin,
    position: Number(card.position || card.sort_order || index + 1),
    sort_order: index,
  }));
}
...
function mergeFlashcardGroupsForDisplay(groups = []) {
  ...
  groups.forEach(({ cards = [], origin = 'original' }) => {
    addFlashcardSource(cards, origin).forEach((card) => {
      const key = buildFlashcardIdentityKey(card);
...
function pickRunFlashcardsForDefaultView(run = {}) {
  ...
  const display = Array.isArray(run.display_flashcards) ? run.display_flashcards : [];

  if (display.length > 0) {
    const cards = addFlashcardSource(display, 'current');
  ...
  const cards = enrichedIsCurrentList
    ? addFlashcardSource(enriched, 'current')
    : mergeFlashcardGroupsForDisplay([
  ...
  return {
    cards,
    origin: enriched.length > 0 ? 'current' : 'original',
    originalCount: original.length,
    enrichedCount: enriched.length,
    totalCount: cards.length,
  };
...
export default function AdvancedFlashcardPoC() {
  ...
  const generateMnemonicFlashcardsFromCurrentRun = async () => {
    ...
    try {
      ...
      const {
        cards: loadedFlashcards,
        origin: loadedFlashcardsOrigin,
      } = pickRunFlashcardsForDefaultView(
        data.run || {
          flashcards,
          enriched_flashcards: data.flashcards || [],
          display_flashcards: data.displayFlashcards || data.display_flashcards || [],
        }
      );
      ...
      setEnrichedFlashcardsGeneratedAt(data.enrichedFlashcardsGeneratedAt || null);
      setFlashcardsViewMode('grid');
      setFlashcardsOrigin(loadedFlashcardsOrigin || 'current');
      setFlashcardsLibrarySaveStatus('idle');
  ...
  const generateFlashcardsFromEnrichedRun = async (
    ...
    try {
      ...
      setFlashcardsViewMode('grid');
      setFlashcardsOrigin(loadedFlashcardsOrigin);
      setFlashcardsLibrarySaveStatus(data.librarySaved ? 'saved' : 'idle');
  ...
  const getCurrentFlashcardStorageOrigin = () => {
    if (
      flashcardsOrigin === 'current' ||
      flashcardsOrigin === 'enriched' ||
      flashcardsOrigin === 'mnemonic' ||
      flashcardsOrigin === 'original'
    ) {
      return flashcardsOrigin;
    }

    return flashcards.length > 0 ? 'current' : 'original';
  };
  ...
  const createFlashcardFromScratch = async () => {
    ...
    try {
      ...
      const newCard = {
        ...
        questionImageUrl: newFlashcardForm.questionImageUrl,
        question_image_url: newFlashcardForm.questionImageUrl,
        questionImageObjectKey: newFlashcardForm.questionImageObjectKey,
        question_image_object_key: newFlashcardForm.questionImageObjectKey,
        answerImageUrl: newFlashcardForm.answerImageUrl,
        answer_image_url: newFlashcardForm.answerImageUrl,
        answerImageObjectKey: newFlashcardForm.answerImageObjectKey,
        answer_image_object_key: newFlashcardForm.answerImageObjectKey,
      };
  ...
  const uploadFlashcardFieldImage = async (index, file, field = 'question') => {
    ...
    try {
      ...
      setFlashcardActionIndex(index);
      setFlashcardImageFieldAction(field);
      setError(null);
      ...
      setFlashcardActionIndex(null);
      setFlashcardImageFieldAction('');
    }
  };

  const renderFlashcardFieldImageUploadButton = ({
    index,
    field,
    label,
    className = '',
    ...
    const isActive =
      isUploadingFlashcardImage &&
      flashcardActionIndex === index &&
      flashcardImageFieldAction === field;

    return (
      <label
        className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer ${className}`}
      >
        {isActive ? (
        ...
        <span>{isActive ? 'Enviando...' : label}</span>
        ...
        <input
          ...
          onChange={(event) => {
            ...
            event.target.value = '';
            uploadFlashcardFieldImage(index, file, field);
          }}
        />
      </label>
    );
  ...
  const generateFlashcardFieldImage = async (index, field = 'question') => {
    if (!currentRunId) {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE}/api/history/${currentRunId}/flashcards/${index}/generate-image`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao gerar imagem do campo.');
      }
  ...
  const renderFlashcardFieldImageActions = ({
    index,
    field,
    uploadLabel,
    aiLabel = 'Gerar imagem IA',
  }) => {
    const isUploading =
      isUploadingFlashcardImage &&
      flashcardActionIndex === index &&
    ...
    const isGenerating =
      isGeneratingFlashcardImage &&
      flashcardActionIndex === index &&
      flashcardImageFieldAction === field;

    return (
      <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full">
        {renderFlashcardFieldImageUploadButton({
          index,
          field,
          label: isUploading ? 'Enviando...' : uploadLabel,
          className: 'w-full sm:flex-1',
        })}

        <button
          type="button"
          onClick={() => generateFlashcardFieldImage(index, field)}
          disabled={isGeneratingFlashcardImage || isUploadingFlashcardImage}
          className="w-full sm:flex-1 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50"
        >
          {isGenerating ? (
            <Loader2 size={16} className="animate-spin" />
          ) : (
          ...
          <span>{isGenerating ? 'Gerando...' : aiLabel}</span>
        </button>
      </div>
    );
  };

  const setNewFlashcardFieldImage = ({ field = 'question', publicUrl = '', key = '' }) => {
    setNewFlashcardForm((prev) => {
      if (field === 'answer') {
        return {
          ...prev,
          answerImageUrl: publicUrl || '',
          answerImageObjectKey: key || '',
        };
      }
      ...
      return {
        ...
        questionImageObjectKey: key || '',
      };
    });
  };

  const getNewFlashcardFieldImage = (field = 'question') => {
    if (field === 'answer') {
      return {
        url: newFlashcardForm.answerImageUrl,
        key: newFlashcardForm.answerImageObjectKey,
      };
    }

    return {
      url: newFlashcardForm.questionImageUrl,
      key: newFlashcardForm.questionImageObjectKey,
    };
  };

  const uploadNewFlashcardFieldImage = async (file, field = 'question') => {
    if (!file) return;

    if (!currentRunId) {
      setError('Abra ou processe um estudo salvo antes de enviar imagem para um novo flashcard.');
      return;
    }
    ...
    try {
      setIsUploadingFlashcardImage(true);
      setNewFlashcardImageFieldAction(`upload:${field}`);
      setError(null);

      const contentType = file.type || 'image/png';

      const uploadResponse = await fetch(
        `${API_BASE}/api/history/${currentRunId}/flashcards/preview-image-upload-url`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      setIsUploadingFlashcardImage(false);
      setNewFlashcardImageFieldAction('');
    }
  };

  const generateNewFlashcardFieldImage = async (field = 'question') => {
    if (!currentRunId) {
      setError('Abra ou processe um estudo salvo antes de gerar imagem para um novo flashcard.');
      return;
    }

    const isQuestion = field === 'question';

    const promptBase = isQuestion
      ? newFlashcardForm.question || premiumEditorHtmlToText(newFlashcardForm.questionHtml)
      : newFlashcardForm.answer || premiumEditorHtmlToText(newFlashcardForm.answerHtml);

    if (!String(promptBase || '').trim()) {
      setError(`Preencha o campo de ${isQuestion ? 'pergunta' : 'resposta'} antes de gerar a imagem.`);
      return;
    }

    try {
      setIsGeneratingFlashcardImage(true);
      setNewFlashcardImageFieldAction(`ai:${field}`);
      setError(null);

      const response = await fetch(
        `${API_BASE}/api/history/${currentRunId}/flashcards/generate-image-preview`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
      ...
      if (!response.ok) {
        throw new Error(data.error || 'Erro ao gerar imagem temporária.');
      }

      setNewFlashcardFieldImage({
        field,
        publicUrl: data.imageUrl || data.publicUrl || '',
        key: data.imageObjectKey || data.key || '',
      });
    } catch (err) {
      setError(`Falha ao gerar imagem com IA: ${err.message}`);
    } finally {
  ...
  const renderNewFlashcardFieldImageActions = ({
    field,
    uploadLabel,
    aiLabel = 'Gerar imagem IA',
  }) => {
    const image = getNewFlashcardFieldImage(field);

    const isUploading =
      isUploadingFlashcardImage &&
      newFlashcardImageFieldAction === `upload:${field}`;

    const isGenerating =
      isGeneratingFlashcardImage &&
      newFlashcardImageFieldAction === `ai:${field}`;

    return (
      <div className="w-full space-y-3">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-3 w-full">
          <label className="w-full sm:flex-1 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors cursor-pointer">
            {isUploading ? (
              <Loader2 size={16} className="animate-spin" />
            ) : (
              <Upload size={16} />
            ...
            <span>{isUploading ? 'Enviando...' : uploadLabel}</span>

            <input
              type="file"
              accept="image/*"
              className="hidden"
              disabled={isUploadingFlashcardImage || isGeneratingFlashcardImage}
              onChange={(event) => {
                const file = event.target.files?.[0];
                event.target.value = '';
                uploadNewFlashcardFieldImage(file, field);
              }}
            />
          </label>
          ...
          <button
            type="button"
            onClick={() => generateNewFlashcardFieldImage(field)}
            disabled={isUploadingFlashcardImage || isGeneratingFlashcardImage}
            className="w-full sm:flex-1 inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors disabled:opacity-50"
          >
            {isGenerating ? (
        ...
        {image.url ? (
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3">
            <img
              src={image.url}
              alt={`Preview da imagem da ${field === 'answer' ? 'resposta' : 'pergunta'}`}
              className="mx-auto max-h-[180px] w-full object-contain rounded-xl"
            />
          </div>
        ) : null}
      </div>
  ...
  const hasUsefulFlashcardInsights = useCallback((card) => {
    ...
    return Boolean(
      String(insights.gap || '').trim() ||
        ...
        String(insights.mnemonic_value || '').trim() ||
        String(insights.mnemonic_application || '').trim() ||
    ...
    try {
      ...
      const mnemonicNote =
        insights.mnemonic_detected && insights.mnemonic_value
          ? [
              `Mnemônico útil: ${insights.mnemonic_value}`,
      ...
      const finalPreceptorNote = [
        insights.preceptor_note_suggestion || '',
        mnemonicNote,
        card.preceptorNote || card.preceptor_note || '',
      ]
        .filter(Boolean)
        .join('\n\n');
      ...
      const finalTags = Array.from(
        new Set([
          ...mergedTags,
          ...(insights.mnemonic_detected ? ['mnemônico'] : []),
        ])
      );
  ...
  const deleteFlashcardAtIndex = async (index) => {
    ...
    try {
      ...
      if (currentStudyIndex >= nextFlashcards.length) {
        setCurrentStudyIndex(Math.max(0, nextFlashcards.length - 1));
      }
    } catch (err) {
    ...
    <div className="min-h-screen bg-[#f5f7fb] font-sans text-slate-800">
      ...
      >
        ...
        >
          <div className="w-11 h-11 rounded-2xl bg-blue-600 text-white flex items-center justify-center shrink-0 shadow-md shadow-blue-200">
            <Glasses size={23} strokeWidth={2.4} />
          </div>
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-6">
            ...
            >
              <div className="min-h-[calc(100vh-96px)] bg-[radial-gradient(circle_at_top,_rgba(37,99,235,0.09),_transparent_34%),linear-gradient(to_bottom,_#ffffff,_#f8fafc)] px-4 py-10 md:py-14">
                <div className="mx-auto max-w-2xl">
                  <div className="text-center mb-7">
                    <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-lg shadow-blue-200">
                      <Glasses size={36} strokeWidth={2.4} />
                    </div>

                    <p className="text-xs font-black uppercase tracking-[0.22em] text-blue-500 mb-2">
                      Medicina JVS
                    </p>
                  ...
                  <div className="mb-6 rounded-2xl border border-blue-100 bg-blue-50 px-5 py-4 text-left shadow-sm">
                          ...
                          >
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-4">
                              ...
                              <button
                                ...
                                className="w-full px-3 py-2 rounded-xl border border-amber-200 bg-amber-50 text-amber-700 text-xs font-bold hover:bg-amber-100 transition-colors disabled:opacity-50"
                              >
                                ...
                                <div className="mt-4 rounded-2xl border border-red-100 bg-red-50/60 p-4">
                                  ...
                                  {expandedFlashcardInsights[index] !== false ? (
                                    <div className="mt-4 space-y-3 text-sm text-red-950/80 leading-relaxed">
                                      ...
                                      {card.cardInsights.mnemonic_detected && card.cardInsights.mnemonic_value ? (
                                        <div className="rounded-xl border border-amber-200 bg-amber-50 p-3">
                                          <p className="text-[10px] font-black uppercase tracking-wider text-amber-700 mb-1">
                                            Mnemônico útil para este card
                                          </p>

                                          <p className="font-bold text-amber-950">
                                            {card.cardInsights.mnemonic_value}
                                          </p>

                                          {card.cardInsights.mnemonic_application ? (
      ...
      {evidenceAnalysis ? 'Com análise' : 'Sem análise'}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <div className="mb-3 flex flex-col items-center justify-center gap-3">
                      ...
                      {renderNewFlashcardFieldImageActions({
                        field: 'question',
                        uploadLabel: 'Imagem da pergunta',
                        aiLabel: 'Gerar imagem IA',
                      })}
                    </div>
                  ...
                  <div className="md:col-span-2">
                    <div className="mb-3 flex flex-col items-center justify-center gap-3">
                      ...
                      {renderNewFlashcardFieldImageActions({
                        field: 'answer',
                        uploadLabel: 'Imagem da resposta',
                        aiLabel: 'Gerar imagem IA',
                      })}
                    </div>
        ...
        {editingFlashcardIndex !== null && (
          <div className="fixed inset-0 z-[90] bg-slate-950/60 backdrop-blur-sm flex items-center justify-center px-4 py-8">
            <div className="w-full max-w-5xl max-h-[92vh] overflow-y-auto no-visible-scrollbar rounded-3xl bg-white shadow-2xl border border-slate-200">
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="md:col-span-2">
                    <div className="mb-3 flex flex-col items-center justify-center gap-3">
                      ...
                      {renderFlashcardFieldImageActions({
                        index: editingFlashcardIndex,
                        field: 'question',
                        uploadLabel: 'Imagem da pergunta',
                        aiLabel: 'Gerar imagem IA',
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 border-t border-slate-100 pt-5">
                  ...
                  <button
                    ...
                    disabled={isGeneratingFlashcardInsights}
                    className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-5 py-3 text-sm font-bold text-amber-700 hover:bg-amber-100 disabled:opacity-50"
                  >
                  ...
                  <button
                    type="button"
                    onClick={closeFlashcardEditor}
                    className="w-full inline-flex items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-5 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50"
                  >
                    Cancelar
                  </button>
                </div>
